...
const CheckOrder: FC<ICheckOrder> = ({
	...
	return (
		<div className="fixed w-screen h-screen overflow-hidden bg-shadow top-0 left-0 z-30 flex justify-center items-center">
			<div className="bg-bg-color border border-brown p-4 lg:p-2 flex flex-col gap-1.5 lg:gap-0.5 rounded-xl z-40 text-black relative">
				<button
				...
				<h3 className="text-2xl lg:text-0.5-lg font-bold ">
					Подтверждение заказа:
				</h3>
				{isAccepted ? (
					<div className="flex flex-col items-center gap-1 text-black text-lg lg:text-0.5-base">
						{isPending ? (
							...
							<>
								<p>
									Ваш заказ
									<span className="text-gray text-xl lg:text-0.5-lg underline px-0.3">
										#{orderID}
									</span>
									успешно оформлен!
								</p>
					...
					<>
						<div className="flex flex-col gap-0.3 text-lg lg:text-sm">
							{cartItems.map(item => (
						...
						<div className="text-lg lg:text-0.5-base">
							на сумму
							<span className="text-xl lg:text-lg ml-0.5 font-bold">
						...
						<div className="flex flex-col w-full items-center gap-0.75 mt-0.75">
							<Button
								...
								onClick={() => mutateAsync()}
							/>
							<div className="underline text-base lg:text-sm w-max uppercase text-gray mx-auto">
